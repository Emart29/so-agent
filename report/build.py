...
def tier_section(cells: list[CellResult]) -> str:
    ...
    return (
        ...
        "<p class='note'><strong>Do not read this table as a tier comparison.</strong> "
        "A model whose provider rejects <code>json_schema</code> contributes rows "
        "only to the weaker tiers, so the strict-mode pool contains more capable "
        "models by construction and wins a contest it never entered. The per-model "
        "rows below are what support a claim about what a tier is worth — and there, "
        "enforcement beats the prompt-only baseline on none of the models tested.</p>"
